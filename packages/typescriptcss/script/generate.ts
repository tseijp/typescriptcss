...
const flat = (css: any) => {
        ...
        const w = (x: any, p: string[]) => { for (const k in x) { const v = x[k]; v && typeof v === 'object' ? w(v, [...p, k]) : o.push([[...p, k].join('\u0001'), String(v)]) } }
        w(css, [])
        return o
}
const unflat = (t: Record<string, string>) => {
        const out: any = {}
        for (const p in t) { const ks = p.split('\u0001'); let o = out; for (let i = 0; i < ks.length - 1; i++) o = o[ks[i]] || (o[ks[i]] = {}); o[ks[ks.length - 1]] = t[p] }
        return out
}
const infer = (node: Node) => {
        ...
        for (const mult of node.kind === 'n' ? MULTS : [null]) {
                ...
                let ok = true
                const [a0, c0] = S[0]
                for (const [p, val] of flat(c0)) { const tok = mult == null ? String(a0) : String(Number(a0) * mult); tpl[p] = tok !== '' && val.split(tok).length > 1 ? val.split(tok).join('^') : val }
                for (const [a, c] of S) {
                        const fl = flat(c)
                        if (fl.length !== Object.keys(tpl).length) { ok = false; break }
                        for (const [p, val] of fl) { const t = tpl[p]; if (t === undefined) { ok = false; break }; const tok = mult == null ? String(a) : String(Number(a) * mult); if (t.split('^').join(tok) !== val) { ok = false; break } }
                        if (!ok) break
                }
                if (ok) { node.mult = mult == null ? 1 : mult; node.rule = unflat(tpl); return }
        }
        node.mult = 1
...
const encRule = (o: any) => (isFlat(o) ? Object.entries(o).map(([k, v]) => kebab(k) + '=' + v).join('|') : '{' + JSON.stringify(o))
const serialize = (root: string) => {
        const lines: string[] = []
        const t = trees[root]
        if ('rule' in t) lines.push('=' + encRule(t.rule))
        const walk = (node: Node, depth: number) => {
                for (const k of Object.keys(node.ch)) { const c = node.ch[k]; let head = '.'.repeat(depth) + k; if (k === '$' && c.mult !== undefined && c.mult !== 1) head += '*' + c.mult; lines.push('rule' in c ? head + '=' + encRule(c.rule) : head); walk(c, depth + 1) }
        }
        walk(t, 0)
        return lines.join(';')
}
...
const decl = (r: string) => {
        const dsl = serialize(r)
        const name = exportName(r)
        const line = `export const ${name}: U['${r}'] = u(\`${dsl}\`)`
...
const body = Object.keys(trees).sort().map(decl).join('\n')
const tail = "\nexport const define = <T = C>(name: string, dsl = ''): T => u(dsl) as unknown as T"
fs.writeFileSync(path.join(dir, 'src', 'index.ts'), head + body + tail + '\n')
console.log('generated src/index.ts roots', Object.keys(trees).length)
