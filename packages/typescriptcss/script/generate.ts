...
for (const [chain, ca, consts, expr] of rows) {
	...
	if (ca && ca[0] === '{') { const a = ev(consts, ca); css = { ...css }; for (const k in a) delete css[k] }
...
const encRule = (css: any, p: string[], pos: number, tok: string | null): string =>
	Object.entries(css).map(([k, v]) => (v && typeof v === 'object' ? k + '{' + encRule(v, p, pos, tok) + '}' : encStr(kebab(k), p, pos, null, false) + ':' + encStr(String(v), p, pos, tok, true))).join('|')
const resolveDots = (piece: string, p: string[], pos: number, tok: string | null) =>
	piece.replace(/\.+/g, (run, idx) => { if (/[0-9]/.test(piece[idx - 1] || '') || /[0-9]/.test(piece[idx + run.length] || '')) return run; if (run.length === 1) return tok != null ? tok : norm(p[pos]); const seg = p[pos - (run.length - 1)]; return seg === '$' || seg === '_' ? (tok as string) : norm(seg) })
...
const decodeCheck = (enc: string, p: string[], pos: number, tok: string | null): any => {
	...
	for (const item of splitTop(enc)) {
		...
		if (b >= 0 && item.endsWith('}')) { o[item.slice(0, b)] = decodeCheck(item.slice(b + 1, -1), p, pos, tok); continue }
...
const occAt = (text: string, sub: string) => {
	...
	while ((i = text.indexOf(sub, i)) >= 0) {
		if (PRE.includes(text[i - 1] || '')) { i++; continue }
		pos.push(i)
		i += sub.length
	}
	return pos
...
const stable = (x: any): string => (x && typeof x === 'object' ? '{' + Object.keys(x).sort().map((k) => JSON.stringify(k) + ':' + stable(x[k])).join(',') + '}' : JSON.stringify(x))
let bad = 0
for (const [chain, ca, consts, expr] of rows) {
	...
	if (stable(got) !== stable(expected)) { bad++; if (bad < 8) process.stdout.write('MISMATCH ' + chain + '(' + ca + ')\n') }
}
if (bad) { process.stdout.write('verify failed: ' + bad + '/' + rows.length + '\n'); process.exit(1) }
