...
const decRule = (enc: string, path: string[], pos: number, tok: string | null): Record<string, any> => {
	...
	for (const item of splitTop(enc)) {
		...
		if (b >= 0 && item.endsWith('}')) { o[item.slice(0, b)] = decRule(item.slice(b + 1, -1), path, pos, tok); continue }
...
const parse = (dsl: string) => {
	...
	for (const raw of dsl.split(';')) {
		if (!raw) continue
		const dots = (raw.match(/^\.*/) as RegExpMatchArray)[0].length, body = raw.slice(dots), eq = body.indexOf('=')
		...
		for (const rawKey of head.split('~')) {
			const short = rawKey.endsWith('?'), kk = short ? rawKey.slice(0, -1) : rawKey
...
const read = (root: Node, name: string, path: string[], start: number) => {
	...
	for (let i = start; i < path.length; i++) {
		const key = path[i]
		let next = node.ch[key], mytok: string | null = null, sk = key
		if (!next) {
