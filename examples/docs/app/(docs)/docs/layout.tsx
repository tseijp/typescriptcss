                                ...
                                <nav style={gap[1].flex.col()}>
                                        {primaryNav.map((item) => (
                                                <a key={item.label} href={item.href} style={px[3].py[2].text[3.5].text[color.muted].gap[3].flex.items.center.font.medium.rounded[1.5].textDecoration.none()}>
                                                        ...
                                                        {'badge' in item ? <span style={px[2].marginLeft.auto.text[2.5].text[color.cyan].rounded[1].border.border[color.cyan]()}>{item.badge}</span> : null}
                                                </a>
                                        ))}
                                </nav>
                                {sections.map((section) => (
                                        <div key={section.title} style={gap[1].flex.col()}>
                                                <span style={px[4].py[2].text[3].text[color.faint].letterSpacing['0.08em'].font.semibold.textTransform.uppercase()}>{section.title}</span>
                                                <div style={flex.col()}>
                                                        {section.items.map((item) => {
                                                                const active = item.href === '/docs'
                                                                return (
                                                                        <a key={item.label} href={item.href} style={px[4].py[1].text[3.5].text[active ? color.text : color.muted].flex.items.center.fontWeight[active ? 600 : 400].border.l.borderLeftWidth['2px'].border[active ? color.cyan : color.border].textDecoration.none()}>
                                                                                {item.label}
                                                                        </a>
                                                                )
                                                        })}
