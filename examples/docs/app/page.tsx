...
const Bento = ({ title, body }: any) => (
        <div style={p[6].gap[3].flex.col.bg[color.panel].rounded[4].border[color.border].backgroundImage[cardSheen.backgroundImage]()}>
                <div style={text[4].text[color.text].font.semibold()}>{title}</div>
                <div style={text[3.5].text[color.muted].leading[7]()}>{body}</div>
        </div>
)
const dot = (color: '#f87171' | '#fbbf24' | '#34d399') => <span style={bg[color].rounded.full.width['11px'].height['11px']()} />

export default function Home() {
        return (
                <div style={w.full.flex.col.items.center()}>
                        <section style={px[6].py[20].w.full.max.w[240].gap[6].flex.col.items.center.backgroundImage['radial-gradient(60% 50% at 50% 0%, rgba(34,211,238,0.12) 0%, transparent 70%)'].position.relative()}>
                                <span style={px[3].py[1].text[3].text[color.cyan].gap[2].flex.items.center.rounded.full.border[color.border].fontFamily[fontMono]()}>v0.1 — TypeScript-authored styles</span>
                                <h1 style={max.w[208].text[14].text[color.text].font.semibold.text.center.lineHeight['1.05'].letterSpacing['-0.03em']()}>Write your styles in TypeScript. Ship them as CSS.</h1>
                        ...
                        <section style={px[6].py[6].w.full.max.w[240].gap[6].flex.items.stretch()}>
                                <div style={min.w[0].flex[2].col()}>
                                        <div style={flex.col.fontSize['13px'].lineHeight['22px'].bg[color.panel].rounded[3].border[color.border].overflow.hidden.fontFamily[fontMono].boxShadow['0 20px 60px -20px rgba(0,0,0,0.6)']()}>
                                                <div style={px[4].py[3].gap[2].flex.items.center.bg[color.panelHi].border.b.borderBottomColor[color.border]()}>
                                                        {dot('#f87171')}
                                                        {dot('#fbbf24')}
                                                        {dot('#34d399')}
                                                ...
                                                <div style={py[4].flex.col()}>
                                                        {[[{ t: 'kw', v: 'export default' }, { v: ' ' }, { t: 'kw', v: 'function' }, { v: ' ' }, { t: 'fn', v: 'Card' }, { v: '() {' }], [{ v: '  ' }, { t: 'kw', v: 'return' }, { v: ' (' }], [{ v: '    <' }, { t: 'fn', v: 'div' }, { v: ' style={' }, { t: 'fn', v: 'p' }, { v: '[' }, { t: 'str', v: '6' }, { v: ']' }, { t: 'mut', v: '.gap' }, { v: '[' }, { t: 'str', v: '4' }, { v: ']' }], [{ v: '      ' }, { t: 'mut', v: '.flex.col.items.center' }], [{ v: '      ' }, { t: 'mut', v: '.bg' }, { v: '[' }, { t: 'str', v: "'#0b1120'" }, { v: ']' }, { t: 'mut', v: '.rounded' }, { v: '[' }, { t: 'str', v: '4' }, { v: ']' }, { t: 'mut', v: '.dark.bg.black' }, { v: '()}>' }], [{ v: '      <' }, { t: 'fn', v: 'h2' }, { v: ' style={' }, { t: 'fn', v: 'text' }, { v: '[' }, { t: 'str', v: "'#fff'" }, { v: ']' }, { t: 'mut', v: '.font.semibold' }, { v: '()}>' }], [{ v: '        Zero runtime' }], [{ v: '      </' }, { t: 'fn', v: 'h2' }, { v: '>' }], [{ v: '    </' }, { t: 'fn', v: 'div' }, { v: '>' }], [{ v: '  )' }], [{ v: '}' }]].map((segments: any[], lineIndex: number) => (
                                                                <div key={lineIndex} style={px[5].flex.items.center.minHeight['22px']()}>
                                ...
                                <div style={min.w[0].flex[1].col.justify.center()}>
                                        <div style={p[6].gap[4].flex.col.items.center.bg['#0b1120'].rounded[4].border[color.border].dark.bg.black()}>
                                                ...
                                                <div style={text['#fff'].fontSize['18px'].font.semibold()}>Zero runtime</div>
                        ...
                        <section style={px[6].py[20].w.full.max.w[240].gap[8].flex.col()}>
                                <div style={gap[3].flex.col()}>
                                        <span style={text[3].text[color.cyan].font.semibold.fontFamily[fontMono].letterSpacing['0.12em'].textTransform.uppercase()}>Why typescriptcss</span>
                                        <h2 style={max.w[176].fontSize['34px'].text[color.text].font.semibold.letterSpacing['-0.02em']()}>The ergonomics of utilities, without the stylesheet to maintain.</h2>
                                </div>
