...
You style things with typescriptcss by combining many single-purpose utilities directly in your markup. Instead of writing class strings, you build a chain of property accesses that resolves to a style object. A chain like `flex.col.items.center.gap[4]` reads like a sentence and produces exactly the declarations it names.

{/* prettier-ignore */}
```tsx
<div style={flex.items.center.gap[4].max.w[96].p[6].rounded[4].bg['#fff']()}>
  <Logo />
  <div>
...
{/* prettier-ignore */}
```tsx
const base = flex.col.gap[3]       // a chain, not yet finalized
...
{/* prettier-ignore */}
```tsx
<button style={bg['#316ff6'].text['#fff'].px[5].py[2].rounded[2]()}>
  Sign in
</button>
```
...
{/* prettier-ignore */}
```tsx
<aside style={flex.col.gap[3].bg['#0b1120']({ position: 'sticky', top: 0 })}>
  {/* ... */}
</aside>
```
...
{/* prettier-ignore */}
```tsx
<div style={{ ...rounded.lg.p[4](), backgroundColor: brandColor }}>
  {/* ... */}
</div>
```
